...
library(hrbrthemes)
library(devEMF)
wd <- paste("/Users/alireza/Library/CloudStorage/Box-Box/GWU/Research/",
            "Longitudinal Review/TeXfiles/",sep = "")
setwd(wd)
pubsight <- read.table("report/pubmed_data.tsv",header = T , sep = '\t')
...
ggplot(pubsight, aes(fill=`Omics related field`, y=Count, x=Year)) + 
  ...
  scale_x_continuous(breaks = round(seq(min(pubsight$Year), max(pubsight$Year), by = 5), 1)) +
  labs(title = "<span style='font-size: 22pt;'><b>a</b></span>") +  # Bold letter "b" only
...
ggsave(paste(wd,"New draft/pubmed_ggplot.eps",sep=""), 
